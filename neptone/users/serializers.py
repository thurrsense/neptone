...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['username', 'email', 'password', 'password_confirm', 'captcha_key', 'captcha_response']
    ...
    def validate(self, attrs):
        ...
        if not user:
            ...
            if not attrs.get('captcha_key') or not attrs.get('captcha_response'):
                raise serializers.ValidationError("Требуется капча после неудачной попытки входа")
            
            # Проверяем капчу
            try:
                captcha = CaptchaStore.objects.get(hashkey=attrs['captcha_key'])
